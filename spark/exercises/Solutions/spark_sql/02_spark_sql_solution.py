...
#	Select the id and title of the wiki article that contain both 'italy' and 'bologna'
# 	in its text


# SELECT COUNT(*) FROM wikiData WHERE text LIKE '%italy%'
# SELECT COUNT(*) FROM wikiData WHERE text LIKE '%bologna%'

italy = sqlCtx.sql("SELECT id, title FROM wikiData WHERE text LIKE '%italy%'").cache()
italy.collect()
bologna = sqlCtx.sql("SELECT id, title FROM wikiData WHERE text LIKE '%bologna%'").cache()
bologna.collect()

bologna.join(italy).collect()